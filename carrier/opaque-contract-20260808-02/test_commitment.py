...
class OpaqueCommitmentTest(unittest.TestCase):
    ...
    def test_bundle_commitment_recomputes(self):
        payload = "".join(f"{label}\0{digest}\n" for label, digest in self.data["artifact_commitments"].items()).encode("utf-8")
        self.assertEqual(hashlib.sha256(payload).hexdigest(), self.data["bundle_sha256"])

    def test_private_ci_unavailable_is_explicit(self):
        receipt = self.data["local_exact_content_validation"]
        self.assertEqual(receipt["private_ci_step_execution"], "unavailable")
        self.assertEqual(receipt["public_commitment_tests_failed"], 0)
    ...
    def test_public_payload_is_opaque(self):
        ...
        forbidden = ("newonly2", "production/", "run_fgi45", "rv_spread", "multiple", "sharpe", "private_key", "sk-")
