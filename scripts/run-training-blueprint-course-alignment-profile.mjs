...
export const TRAINING_BLUEPRINT_COURSE_ALIGNMENT_EXACT_FILES = new Set([
  'docs/architecture/trainingos-training-blueprint-course-alignment-v1.md',
  'packages/training-industry-role-pack-core/src/course-design-alignment.mjs',
  'packages/training-industry-role-pack-core/test/course-design-alignment.test.mjs',
  'tests/test_trainingos_training_blueprint_course_alignment_v1.py',
]);
...
export const trainingBlueprintCourseAlignmentCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('alignment-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/src/course-design-alignment.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/course-design-alignment.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_training_blueprint_course_alignment_v1'], 'python'),
...
export function isTrainingBlueprintCourseAlignmentScope(files) {
  const names = [...files];
  return names.length === TRAINING_BLUEPRINT_COURSE_ALIGNMENT_EXACT_FILES.size
    && names.every((name) => TRAINING_BLUEPRINT_COURSE_ALIGNMENT_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: trainingBlueprintCourseAlignmentCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunTrainingBlueprintCourseAlignmentProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isTrainingBlueprintCourseAlignmentScope(files)) return null;
  ...
  try {
    for (const [index, item] of trainingBlueprintCourseAlignmentCommands.entries()) {
  ...
  const stepCount = trainingBlueprintCourseAlignmentCommands.length;
