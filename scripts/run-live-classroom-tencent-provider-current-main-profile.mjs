...
export const LIVE_CLASSROOM_TENCENT_PROVIDER_EXACT_FILES = new Set([
...
export const liveClassroomTencentProviderCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_live_classroom_tencent_provider_v1'], 'python'),
...
export function isLiveClassroomTencentProviderScope(files) {
  const names = [...files];
  return names.length === LIVE_CLASSROOM_TENCENT_PROVIDER_EXACT_FILES.size
    && names.every((name) => LIVE_CLASSROOM_TENCENT_PROVIDER_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: liveClassroomTencentProviderCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunLiveClassroomTencentProviderCurrentMainProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isLiveClassroomTencentProviderScope(files)) return null;
  ...
  try {
    for (const [index, item] of liveClassroomTencentProviderCommands.entries()) {
  ...
  const stepCount = liveClassroomTencentProviderCommands.length;
