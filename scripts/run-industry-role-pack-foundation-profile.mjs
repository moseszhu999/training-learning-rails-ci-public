...
export const INDUSTRY_ROLE_PACK_EXACT_FILES = new Set([
  'docs/architecture/trainingos-industry-role-pack-foundation-v1.md',
  'packages/training-industry-role-pack-core/package.json',
  'packages/training-industry-role-pack-core/src/index.mjs',
  'packages/training-industry-role-pack-core/test/role-pack-core.test.mjs',
  'tests/test_trainingos_industry_role_pack_foundation_v1.py',
]);
...
export const industryRolePackCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('package-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/src/index.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/role-pack-core.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_industry_role_pack_foundation_v1'], 'python'),
...
export function isIndustryRolePackScope(files) {
  const names = [...files];
  return names.length === INDUSTRY_ROLE_PACK_EXACT_FILES.size
    && names.every((name) => INDUSTRY_ROLE_PACK_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: industryRolePackCommands.length, passedStepCount: 0,
    nodeTests: 0, nodePassed: 0, nodeFailed: 0, pythonTests: 0,
    selectedSuite: 'industry-role-pack-foundation',
  };
}

export async function maybeRunIndustryRolePackFoundationProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isIndustryRolePackScope(files)) return null;
  ...
  try {
    for (const [index, item] of industryRolePackCommands.entries()) {
  ...
  const stepCount = industryRolePackCommands.length;
  ...
  return {
    ...
    nodeTests, nodePassed, nodeFailed, pythonTests,
    selectedSuite: 'industry-role-pack-foundation',
  };
}
