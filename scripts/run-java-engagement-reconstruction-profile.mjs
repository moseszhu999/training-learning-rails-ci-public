...
export const JAVA_ENGAGEMENT_RECONSTRUCTION_EXACT_FILES = new Set([
  'docs/architecture/trainingos-java-engagement-reconstruction-v1.md',
  'packages/training-industry-role-pack-core/reference-pilots/java-engagement-reconstruction-v1.json',
  'packages/training-industry-role-pack-core/src/engagement-reconstruction.mjs',
  'packages/training-industry-role-pack-core/test/engagement-reconstruction.test.mjs',
  'tests/test_trainingos_java_engagement_reconstruction_v1.py',
]);
...
export const javaEngagementReconstructionCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('contract-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/src/engagement-reconstruction.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/engagement-reconstruction.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_java_engagement_reconstruction_v1'], 'python'),
...
export function isJavaEngagementReconstructionScope(files) {
  const names = [...files];
  return names.length === JAVA_ENGAGEMENT_RECONSTRUCTION_EXACT_FILES.size
    && names.every((name) => JAVA_ENGAGEMENT_RECONSTRUCTION_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: javaEngagementReconstructionCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunJavaEngagementReconstructionProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isJavaEngagementReconstructionScope(files)) return null;
  ...
  const failedLabels = [];
  try {
    for (const [index, item] of javaEngagementReconstructionCommands.entries()) {
  ...
  const stepCount = javaEngagementReconstructionCommands.length;
