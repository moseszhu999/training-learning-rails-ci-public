...
export const CAPABILITY_INITIATIVE_EXACT_FILES = new Set([
  'docs/architecture/trainingos-capability-initiative-v1.md',
  'packages/training-industry-role-pack-core/reference-pilots/capability-initiative-reference-cases-v1.json',
  'packages/training-industry-role-pack-core/src/capability-initiative.mjs',
  'packages/training-industry-role-pack-core/test/capability-initiative.test.mjs',
  'tests/test_trainingos_capability_initiative_v1.py',
]);
...
export const capabilityInitiativeCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('contract-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/src/capability-initiative.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/capability-initiative.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_capability_initiative_v1'], 'python'),
...
export function isCapabilityInitiativeScope(files) {
  const names = [...files];
  return names.length === CAPABILITY_INITIATIVE_EXACT_FILES.size
    && names.every((name) => CAPABILITY_INITIATIVE_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: capabilityInitiativeCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunCapabilityInitiativeProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isCapabilityInitiativeScope(files)) return null;
  ...
  try {
    for (const [index, item] of capabilityInitiativeCommands.entries()) {
  ...
  const stepCount = capabilityInitiativeCommands.length;
