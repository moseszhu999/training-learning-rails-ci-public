...
[[ "$migration_start" == 20260729230000 && "$migration_end" == 20260729235959 ]]
...
expected_non_migrations="$(printf '%s\n' \
  docs/architecture/trainingos-challenge-offer-entitlement-v1.md \
  docs/verification/trainingos-challenge-offer-entitlement-v1.md \
  ...
  tests/sql/trainingos_challenge_offer_entitlement_v1_e2e.sql \
  tests/sql/trainingos_challenge_offer_entitlement_v1_e2e_runner.sql \
  tests/test_trainingos_challenge_offer_entitlement_v1_contract.py | sort)"
expected_migrations=(
  supabase/migrations/20260729230000_trainingos_challenge_offer_helpers_v1.sql
  supabase/migrations/20260729230100_trainingos_challenge_offer_catalog_v1.sql
  supabase/migrations/20260729230200_trainingos_challenge_offer_lifecycle_schema_v1.sql
  supabase/migrations/20260729230300_trainingos_challenge_offer_schema_acl_v1.sql
  supabase/migrations/20260729231000_trainingos_challenge_offer_eligibility_resolver_v1.sql
  supabase/migrations/20260729231010_trainingos_challenge_offer_authoring_rpc_v1.sql
  supabase/migrations/20260729231020_trainingos_challenge_offer_publish_rpc_v1.sql
  supabase/migrations/20260729231030_trainingos_challenge_offer_eligibility_rpc_v1.sql
  supabase/migrations/20260729231100_trainingos_challenge_offer_application_rpc_v1.sql
  supabase/migrations/20260729231110_trainingos_challenge_offer_decision_rpc_v1.sql
  supabase/migrations/20260729231200_trainingos_challenge_entitlement_lifecycle_rpc_v1.sql
  supabase/migrations/20260729231300_trainingos_challenge_entitlement_transfer_enrollment_rpc_v1.sql
  supabase/migrations/20260729231400_trainingos_challenge_offer_projection_rpc_v1.sql
  supabase/migrations/20260729231500_trainingos_challenge_offer_rpc_acl_v1.sql
)
...
runner_sql="$PRIVATE_REPO_PATH/tests/sql/trainingos_challenge_offer_entitlement_v1_e2e_runner.sql"

fresh_project="$RUNNER_TEMP/trainingos-challenge-offer-fresh"
upgrade_project="$RUNNER_TEMP/trainingos-challenge-offer-upgrade"
base_worktree="$RUNNER_TEMP/trainingos-challenge-offer-base"
cleanup(){
  ...
  rm -f "$RUNNER_TEMP"/trainingos-challenge-offer-*-status.env
}
trap cleanup EXIT
...
grep -Fq '\ir trainingos_challenge_offer_entitlement_v1_e2e.sql' "$runner_sql"
grep -Eiq '^[[:space:]]*rollback;' "$runner_sql"

CURRENT_STAGE="migration-contract"
mapfile -t migrations < <(
  grep -E '^supabase/migrations/2026072923[0-9]{4}_trainingos_challenge_[^/]+\.sql$' <<<"$changed_files" | sort || true
)
...
fresh_status="$RUNNER_TEMP/trainingos-challenge-offer-fresh-status.env"
...
upgrade_status="$RUNNER_TEMP/trainingos-challenge-offer-upgrade-status.env"
...
echo "CHALLENGE_DATABASE status=PASS suite=offer changed_migrations=${#migrations[@]} source=$source_migration_count generated=$generated_migration_count fresh=PASS second_pass=PASS upgrade=PASS e2e=PASS cleanup=PASS"
