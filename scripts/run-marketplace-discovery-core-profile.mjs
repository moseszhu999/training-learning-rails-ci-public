...
const WORKSPACE_TRANSFER_EXPECTED_NODE_COUNT = 9;
...
export const marketplaceContractsFinderCoreCommands = Object.freeze([
  ...
  command('contracts-finder-tests', 'node', ['--test', 'tests/training-marketplace-live-ingestion-v1.test.mjs', 'tests/training-marketplace-contracts-finder-source-trust-v1.test.mjs'], 'node'),
  command('typecheck', 'npm', ['run', 'typecheck']),
  command('production-build', 'npx', ['vite', 'build', '--config', 'vite.config.ts']),
...
export const marketplaceFindATenderRegistryCommands = Object.freeze([
  ...
  command('find-a-tender-registry-tests', 'node', ['--test', 'tests/training-marketplace-find-a-tender-v1.test.mjs', 'tests/training-marketplace-live-ingestion-sources-v1.test.mjs'], 'node'),
  command('typecheck', 'npm', ['run', 'typecheck']),
  command('production-build', 'npx', ['vite', 'build', '--config', 'vite.config.ts']),
  command('postbuild-copy', 'node', ['scripts/copy-trainingos-marketplace-web.mjs']),
  command('bundle-verification', 'npm', ['run', 'verify:build']),
...
function parseNode(text) { return [...String(text).matchAll(/^# tests\s+(\d+)$/gm)].reduce((sum, match) => sum + Number(match[1]), 0); }
function parseNodePassed(text) { return [...String(text).matchAll(/^# pass\s+(\d+)$/gm)].reduce((sum, match) => sum + Number(match[1]), 0); }
...
export function isMarketplaceDiscoveryCoreScope(files) { return exactSetMatch(files, MARKETPLACE_DISCOVERY_CORE_EXACT_FILES); }
export function isMarketplaceContractsFinderCoreScope(files) { return exactSetMatch(files, MARKETPLACE_CONTRACTS_FINDER_CORE_EXACT_FILES); }
export function isMarketplaceFindATenderRegistryScope(files) { return exactSetMatch(files, MARKETPLACE_FIND_A_TENDER_REGISTRY_EXACT_FILES); }
export function isMarketplaceSourceHealthCoreScope(files) { return exactSetMatch(files, MARKETPLACE_SOURCE_HEALTH_CORE_EXACT_FILES); }
export function isMarketplaceWorkspaceTransferCoreScope(files) { return exactSetMatch(files, MARKETPLACE_WORKSPACE_TRANSFER_CORE_EXACT_FILES); }
...
function failedContractResult(config) {
  return { ok: false, status: 'FAIL:fixed-input-contract', failedLabels: Object.freeze(['fixed-input-contract']), stepCount: config.commands.length, passedStepCount: 0, nodeTests: 0, nodePassed: 0, nodeFailed: 0, pythonTests: 0, selectedSuite: config.selectedSuite };
}
async function runFixedScope(input, scope, config) {
  ...
  try {
    for (const [index, item] of config.commands.entries()) {
      ...
      const result = spawnSync(item.executable, item.args, {
        ...
        env: { ...process.env, PRIVATE_REPO_PATH: input.privateRepoPath, PRIVATE_EXACT_SHA: input.privateExactSha, EXPECTED_MIGRATION_COUNT: String(config.migrationCount), RUNNER_TEMP: input.runnerTemp },
        stdio: ['ignore', descriptor, descriptor],
        shell: false,
      });
      closeSync(descriptor);
      const output = await readFile(logPath, 'utf8');
      if (item.kind === 'node') { nodeTests += parseNode(output); nodePassed += parseNodePassed(output); }
      if (result.status === 0) passedStepCount += 1; else failedLabels.push(item.label);
    }
  } finally {
    await rm(path.join(input.runnerTemp, 'trainingos-scope-contract.env'), { force: true });
  }
  ...
  return { ok, status: ok ? 'PASS' : `FAIL:${failure}`, failedLabels: Object.freeze([...failedLabels]), stepCount: config.commands.length, passedStepCount, nodeTests, nodePassed, nodeFailed: nodeTests - nodePassed, pythonTests: 0, selectedSuite: config.selectedSuite };
}

const historicalConfig = Object.freeze({ commands: marketplaceDiscoveryCoreCommands, migrationCount: CANONICAL_MIGRATION_COUNT, nodeCount: EXPECTED_NODE_COUNT, pythonCount: EXPECTED_PYTHON_COUNT, changedFileCount: 5, selectedSuite: 'marketplace-discovery-core' });
const contractsFinderConfig = Object.freeze({ commands: marketplaceContractsFinderCoreCommands, migrationCount: CONTRACTS_FINDER_CANONICAL_MIGRATION_COUNT, nodeCount: CONTRACTS_FINDER_EXPECTED_NODE_COUNT, pythonCount: CONTRACTS_FINDER_EXPECTED_PYTHON_COUNT, changedFileCount: 5, selectedSuite: 'marketplace-contracts-finder-core' });
const findATenderRegistryConfig = Object.freeze({ commands: marketplaceFindATenderRegistryCommands, migrationCount: FIND_A_TENDER_CANONICAL_MIGRATION_COUNT, nodeCount: FIND_A_TENDER_EXPECTED_NODE_COUNT, pythonCount: FIND_A_TENDER_EXPECTED_PYTHON_COUNT, changedFileCount: 7, selectedSuite: 'marketplace-find-a-tender-registry' });
const sourceHealthConfig = Object.freeze({ commands: marketplaceSourceHealthCoreCommands, migrationCount: SOURCE_HEALTH_CANONICAL_MIGRATION_COUNT, nodeCount: SOURCE_HEALTH_EXPECTED_NODE_COUNT, pythonCount: SOURCE_HEALTH_EXPECTED_PYTHON_COUNT, changedFileCount: 4, selectedSuite: 'marketplace-source-health-core' });
const workspaceTransferConfig = Object.freeze({ commands: marketplaceWorkspaceTransferCoreCommands, migrationCount: WORKSPACE_TRANSFER_CANONICAL_MIGRATION_COUNT, nodeCount: WORKSPACE_TRANSFER_EXPECTED_NODE_COUNT, pythonCount: WORKSPACE_TRANSFER_EXPECTED_PYTHON_COUNT, changedFileCount: 5, selectedSuite: 'marketplace-workspace-transfer-core' });
