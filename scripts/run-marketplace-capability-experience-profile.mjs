...
export const MARKETPLACE_CAPABILITY_EXPERIENCE_EXACT_FILES = new Set([
  'docs/architecture/trainingos-marketplace-capability-experience-v1.md',
  'packages/training-marketplace-capability-experience/package.json',
  'packages/training-marketplace-capability-experience/src/index.d.ts',
  'packages/training-marketplace-capability-experience/src/index.mjs',
  'tests/training-marketplace-capability-experience-v1.test.mjs',
]);
...
export const marketplaceCapabilityExperienceCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('contract-syntax', 'node', ['--check', 'packages/training-marketplace-capability-experience/src/index.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'tests/training-marketplace-capability-experience-v1.test.mjs'], 'node'),
...
export function isMarketplaceCapabilityExperienceScope(files) {
  const names = [...files];
  return names.length === MARKETPLACE_CAPABILITY_EXPERIENCE_EXACT_FILES.size
    && names.every((name) => MARKETPLACE_CAPABILITY_EXPERIENCE_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: marketplaceCapabilityExperienceCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunMarketplaceCapabilityExperienceProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isMarketplaceCapabilityExperienceScope(files)) return null;
  ...
  try {
    for (const [index, item] of marketplaceCapabilityExperienceCommands.entries()) {
  ...
  const countsPassed = nodeTests === EXPECTED_NODE_COUNT
    ...
    && nodeFailed === 0;
  const stepCount = marketplaceCapabilityExperienceCommands.length;
