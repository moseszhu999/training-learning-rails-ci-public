...
export const ORGANIZATION_MEMBERSHIP_PERSISTENCE_EXACT_FILES = new Set([
...
export const organizationMembershipPersistenceCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('focused-node-contracts', 'node', ['--test', 'tests/training-organization-membership-persistence-v1.test.mjs'], 'node'),
...
export function isOrganizationMembershipPersistenceScope(files) {
  const names = [...files];
  return names.length === ORGANIZATION_MEMBERSHIP_PERSISTENCE_EXACT_FILES.size
    && names.every((name) => ORGANIZATION_MEMBERSHIP_PERSISTENCE_EXACT_FILES.has(name));
}
...
function failedContractResult() {
  return {
    ...
    stepCount: organizationMembershipPersistenceCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunOrganizationMembershipPersistenceProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isOrganizationMembershipPersistenceScope(files)) return null;
  ...
  try {
    for (const [index, item] of organizationMembershipPersistenceCommands.entries()) {
      const logPath = path.join(input.runnerTemp, `trainingos-organization-membership-persistence-${index + 1}.log`);
  ...
  const stepCount = organizationMembershipPersistenceCommands.length;
