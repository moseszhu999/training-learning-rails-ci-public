...
export const LIVE_DISCOVERY_PERSISTENCE_EXACT_FILES = new Set([
  'supabase/migrations/20260806233000_trainingos_marketplace_live_discovery_v1.sql',
  'supabase/migrations/20260806234500_trainingos_marketplace_live_discovery_hardening_v1.sql',
  'supabase/migrations/20260807002000_trainingos_marketplace_source_health_v1.sql',
  'tests/test_trainingos_marketplace_live_discovery_persistence_v1.py',
]);
...
const MIGRATION_START = '20260806233000';
const MIGRATION_END = '20260807002000';
...
export const liveDiscoveryPersistenceCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command(
    'focused-python-contracts',
    'python',
    ['-m', 'unittest', '-v', 'tests.test_trainingos_marketplace_live_discovery_persistence_v1'],
    'python',
  ),
...
export function isLiveDiscoveryPersistenceScope(files) {
  const names = [...files];
  return names.length === LIVE_DISCOVERY_PERSISTENCE_EXACT_FILES.size
    && names.every((name) => LIVE_DISCOVERY_PERSISTENCE_EXACT_FILES.has(name));
}
...
function failedContractResult() {
  return {
    ...
    stepCount: liveDiscoveryPersistenceCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
    pythonTests: 0,
    selectedSuite: 'marketplace-live-discovery-persistence',
  };
}

export async function maybeRunLiveDiscoveryPersistenceProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isLiveDiscoveryPersistenceScope(files)) return null;
  ...
  try {
    for (const [index, item] of liveDiscoveryPersistenceCommands.entries()) {
      const logPath = path.join(input.runnerTemp, `trainingos-live-discovery-persistence-${index + 1}.log`);
  ...
  const stepCount = liveDiscoveryPersistenceCommands.length;
