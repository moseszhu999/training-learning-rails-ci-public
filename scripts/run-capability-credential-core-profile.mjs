...
export const CAPABILITY_CREDENTIAL_CORE_EXACT_FILES = new Set([
  'docs/architecture/trainingos-portable-human-capability-credential-v1.md',
  'packages/training-capability-credential-core/package.json',
  'packages/training-capability-credential-core/src/index.d.ts',
  'packages/training-capability-credential-core/src/index.mjs',
  'tests/training-capability-credential-core-v1.test.mjs',
]);
...
export const capabilityCredentialCoreCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('contract-syntax', 'node', ['--check', 'packages/training-capability-credential-core/src/index.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'tests/training-capability-credential-core-v1.test.mjs'], 'node'),
...
export function isCapabilityCredentialCoreScope(files) {
  const names = [...files];
  return names.length === CAPABILITY_CREDENTIAL_CORE_EXACT_FILES.size
    && names.every((name) => CAPABILITY_CREDENTIAL_CORE_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: capabilityCredentialCoreCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunCapabilityCredentialCoreProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isCapabilityCredentialCoreScope(files)) return null;
  ...
  try {
    for (const [index, item] of capabilityCredentialCoreCommands.entries()) {
  ...
  const countsPassed = nodeTests === EXPECTED_NODE_COUNT
    ...
    && nodeFailed === 0;
  const stepCount = capabilityCredentialCoreCommands.length;
