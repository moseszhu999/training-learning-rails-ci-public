export * from './run-private-profile-core.mjs';

import { appendFile, readFile } from 'node:fs/promises';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { runProfile } from './run-private-profile-core.mjs';
import {
  sanitizeBuildSubstage,
  sanitizeTypeScriptDiagnostics,
} from './sanitize-challenge-web-diagnostics.mjs';

async function readSealedLog(runnerTemp, index) {
  try {
...
async function main() {
  ...
  if (profile === 'challenge-web') {
    if (result.failedLabels.includes('typecheck')) {
      typecheckDiagnostics = sanitizeTypeScriptDiagnostics(await readSealedLog(runnerTemp, 3));
    }
    if (result.failedLabels.includes('production-build')) {
      buildSubstage = sanitizeBuildSubstage(await readSealedLog(runnerTemp, 4));
    }
  }
