...
const command = (label, executable, args, kind = 'status', env = {}) => ({
  label,
  executable,
  args,
  kind,
...
function walk(dir){
  ...
  for(const e of fs.readdirSync(dir,{withFileTypes:true})){
    ...
    if(e.isDirectory())walk(p);
      ...
      if(!/^#\s+\S/m.test(t)||/\u0000/.test(t))process.exit(2);
      count++;
    }
  }
}
...
console.log('MARKDOWN_CONTRACT status=PASS files='+count);
`;

const canonicalChallengeCommands = [
  command('install', 'npm', ['ci']),
  command('syntax-definition', 'node', ['--check', 'packages/training-challenge/src/definition.mjs']),
  command('syntax-runtime', 'node', ['--check', 'packages/training-challenge/src/runtime.mjs']),
  command('syntax-package', 'node', ['--check', 'packages/training-challenge/src/index.mjs']),
  command('syntax-gateway', 'node', ['--check', 'lib/trainingos-agent-gateway/challenge-runtime.mjs']),
  command('syntax-mcp', 'node', ['--check', 'lib/trainingos-agent-gateway/mcp-challenge-runtime-layer.mjs']),
  ...
  command('node-contract', 'node', [
    '--test',
    'prototypes/trainingos-agent-mvp-v1/test/challenge-runtime-v1.test.mjs',
    'prototypes/trainingos-agent-mvp-v1/test/challenge-gateway-mcp-v1.test.mjs',
  ], 'node'),
  command('python-contract', 'python', [
    '-m',
    'unittest',
    '-v',
    'tests.test_trainingos_challenge_runtime_v1_contract',
...
export const profileCommands = Object.freeze({
  ...
  'challenge-runtime': canonicalChallengeCommands,
  'challenge-web': [
    command('install', 'npm', ['ci']),
  ...
  'teacher-hub': [
    ...
    command('production-build', 'npx', ['vite', 'build', '--config', 'vite.config.ts']),
    command('playwright-browser', 'npx', ['playwright', 'install', '--with-deps', 'chromium']),
    command('playwright', 'npx', [
      'playwright',
      'test',
      '--config=tests/trainingos-ui-e2e/teacher-operations-hub.config.ts',
    ]),
  ],
  'docs-launch': [command('markdown-contract', 'node', ['-e', markdownContract])],
  'generic-owned': [
    command('install', 'npm', ['ci']),
...
function fixedChallengeCommands(files) {
  ...
  if (selected.length !== 1) throw new Error('unsupported challenge suite');

  const common = [command('install', 'npm', ['ci'])];
  const tail = [
    command('typecheck', 'npm', ['run', 'typecheck']),
    command('production-build', 'npm', ['run', 'build']),
  ];

  if (selected[0] === 'canonical') return canonicalChallengeCommands;
  if (selected[0] === 'evaluation') {
    return [
      ...common,
  ...
  return [
    ...common,
    command('syntax-package', 'node', ['--check', 'packages/training-invite-growth/src/index.mjs']),
    command('syntax-gateway', 'node', ['--check', 'lib/trainingos-agent-gateway/invite-growth-runtime.mjs']),
    command('syntax-concurrency', 'bash', ['-n', 'scripts/run-trainingos-invite-growth-concurrency-e2e.sh']),
    command('node-contract', 'node', ['--test', 'prototypes/trainingos-agent-mvp-v1/test/invite-growth-runtime-v1.test.mjs'], 'node'),
    command('python-contract', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_invite_growth_runtime_v1_contract'], 'python'),
    ...tail,
    command('database-replay', 'bash', [path.join(publicRoot, 'scripts/run-challenge-runtime-database.sh')]),
  ];
}
...
export async function runProfile({
  ...
  if (profile === 'challenge-runtime') {
    ...
    commands = fixedChallengeCommands(result.stdout.trim() ? result.stdout.trim().split('\n') : []);
  }
  ...
  try {
    for (const [index, item] of commands.entries()) {
      ...
      if (item.kind === 'python' || item.kind === 'mixed') {
