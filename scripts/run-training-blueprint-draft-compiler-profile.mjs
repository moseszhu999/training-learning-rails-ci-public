...
export const TRAINING_BLUEPRINT_DRAFT_COMPILER_EXACT_FILES = new Set([
  'docs/architecture/trainingos-training-blueprint-draft-compiler-v1.md',
  'packages/training-industry-role-pack-core/src/training-blueprint.mjs',
  'packages/training-industry-role-pack-core/test/training-blueprint.test.mjs',
  'tests/test_trainingos_training_blueprint_draft_compiler_v1.py',
]);
...
export const trainingBlueprintDraftCompilerCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('compiler-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/src/training-blueprint.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/training-blueprint.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_training_blueprint_draft_compiler_v1'], 'python'),
...
export function isTrainingBlueprintDraftCompilerScope(files) {
  const names = [...files];
  return names.length === TRAINING_BLUEPRINT_DRAFT_COMPILER_EXACT_FILES.size
    && names.every((name) => TRAINING_BLUEPRINT_DRAFT_COMPILER_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: trainingBlueprintDraftCompilerCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunTrainingBlueprintDraftCompilerProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isTrainingBlueprintDraftCompilerScope(files)) return null;
  ...
  try {
    for (const [index, item] of trainingBlueprintDraftCompilerCommands.entries()) {
  ...
  const stepCount = trainingBlueprintDraftCompilerCommands.length;
