...
[[ -f "$scope_file" ]] || { echo "MARKETPLACE_CLAIM_REVIEW_DB status=FAIL stage=scope-contract"; exit 2; }
...
sanitized_failure_marker(){
  ...
  line="$(grep -Eo 'trainingos_marketplace_claim_review_lifecycle_v1_e2e\.sql:[0-9]+' "$log_path" 2>/dev/null \
...
sanitized_start_failure_marker(){
  ...
  if grep -q "$migration_name" "$log_path" 2>/dev/null; then migration_scope="claim_review"
...
start_with_marker(){
  ...
  start_log="$RUNNER_TEMP/trainingos-marketplace-claim-review-${label}.log"
  set +e
...
run_e2e(){
  ...
  e2e_log="$RUNNER_TEMP/trainingos-marketplace-claim-review-${label}-sql-e2e.log"
  set +e
...
where employee_or_student_number in (
  'MKT-C-O-001', 'MKT-C-R-001', 'MKT-C-C-001', 'MKT-C-T-001'
);
select 'claim_decisions=' || count(*)
...
SQL
  ...
  grep -qx 'claim_decisions=0' "$residue_log"
  grep -qx 'fixture_claims=0' "$residue_log"
...
where namespace.nspname = 'private'
  and proc.proname in (
    'trainingos_marketplace_claim_source_host_v1',
    'trainingos_marketplace_claim_review_immutable_guard_v1'
  )
  and proc.prosecdef
  and coalesce(array_to_string(proc.proconfig, ','), '') like '%search_path=%';
...
select 'upstream_owners=' || count(*)
from pg_catalog.pg_class relation
join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
where namespace.nspname = 'public'
  and relation.relname in (
    'trainingos_marketplace_claim_requests',
    'trainingos_marketplace_reviewer_authority_events'
  );

select 'forbidden_adjacent_rpcs=' || count(*)
from pg_catalog.pg_proc
where proname in (
  'create_trainingos_marketplace_supply_from_claim_v1',
  'create_trainingos_marketplace_onboarding_v1',
  'verify_trainingos_marketplace_public_signal_ownership_v1'
);
SQL
  grep -qx 'tables=1' "$catalog_log"
  grep -qx 'public_rpcs=3' "$catalog_log"
  grep -qx 'private_helpers=2' "$catalog_log"
  grep -qx 'authenticated_table_privileges=0' "$catalog_log"
  grep -qx 'elevated_table_privileges=0' "$catalog_log"
  grep -qx 'authenticated_public_rpc=3' "$catalog_log"
  grep -qx 'forbidden_public_rpc_exec=0' "$catalog_log"
  grep -qx 'upstream_owners=2' "$catalog_log"
  grep -qx 'forbidden_adjacent_rpcs=0' "$catalog_log"
}
...
echo "MARKETPLACE_CLAIM_REVIEW_DB status=PASS exact_head=$PRIVATE_EXACT_SHA canonical_migrations=$canonical_migration_count fresh_replay=PASS second_replay=PASS upgrade_replay=PASS sql_e2e=PASS rollback=PASS catalog=PASS zero_residue=PASS cleanup=PASS"
