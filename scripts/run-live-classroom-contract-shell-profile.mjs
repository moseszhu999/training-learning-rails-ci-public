...
export function isLiveClassroomContractShellScope(files) {
  const names = [...files];
  return names.length === LIVE_CLASSROOM_CONTRACT_SHELL_EXACT_FILES.size
    && names.every((name) => LIVE_CLASSROOM_CONTRACT_SHELL_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: liveClassroomContractShellCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
    pythonTests: 0,
    selectedSuite: 'live-classroom-contract-shell',
  };
}

export async function maybeRunLiveClassroomContractShellProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isLiveClassroomContractShellScope(files)) return null;
  ...
  try {
    for (const [index, item] of liveClassroomContractShellCommands.entries()) {
  ...
  const stepCount = liveClassroomContractShellCommands.length;
