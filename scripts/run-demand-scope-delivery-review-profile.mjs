...
export const DEMAND_SCOPE_DELIVERY_REVIEW_EXACT_FILES = new Set([
  'docs/architecture/trainingos-demand-scope-delivery-review-v1.md',
  'packages/training-industry-role-pack-core/reference-pilots/java-demand-scope-delivery-review-v1.json',
  'packages/training-industry-role-pack-core/src/demand-scope-delivery-review.mjs',
  'packages/training-industry-role-pack-core/test/demand-scope-delivery-review.test.mjs',
  'tests/test_trainingos_demand_scope_delivery_review_v1.py',
]);
...
export const demandScopeDeliveryReviewCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('contract-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/src/demand-scope-delivery-review.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/demand-scope-delivery-review.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_demand_scope_delivery_review_v1'], 'python'),
...
export function isDemandScopeDeliveryReviewScope(files) {
  const names = [...files];
  return names.length === DEMAND_SCOPE_DELIVERY_REVIEW_EXACT_FILES.size
    && names.every((name) => DEMAND_SCOPE_DELIVERY_REVIEW_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: demandScopeDeliveryReviewCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunDemandScopeDeliveryReviewProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isDemandScopeDeliveryReviewScope(files)) return null;
  ...
  try {
    for (const [index, item] of demandScopeDeliveryReviewCommands.entries()) {
  ...
  const stepCount = demandScopeDeliveryReviewCommands.length;
