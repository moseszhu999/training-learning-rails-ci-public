...
if grep -q '^packages/training-challenge/src/' <<<"$changed_files"; then
  suite=canonical
  ...
  echo 'CHALLENGE_DATABASE status=FAIL reason=unsupported-suite'
  exit 2
fi
...
preflight(){
  ...
  local actual_count canonical_count
  ...
  echo "CHALLENGE_DATABASE stage=preflight status=PASS suite=$suite migrations=${#migrations[@]} canonical=$canonical_count"
}
...
PY
  ...
  rm -rf "$fresh_project"
  echo "CHALLENGE_DATABASE stage=fresh status=PASS suite=$suite first=PASS second=PASS e2e=PASS cleanup=PASS"
}
...
upgrade_replay(){
  ...
  mapfile -t migrations < <(grep -E '^supabase/migrations/[0-9]{14}_[^/]*(challenge|invite|growth|attribution|proof|sharing)[^/]*\.sql$' <<<"$changed_files"|sort)
  for migration in "${migrations[@]}"; do cp "$PRIVATE_REPO_PATH/$migration" "$upgrade_project/supabase/migrations/"; done
  ...
  rm -rf "$upgrade_project" "$base_worktree"
  echo "CHALLENGE_DATABASE stage=upgrade status=PASS suite=$suite migration_up=PASS e2e=PASS cleanup=PASS"
}

case "$mode" in
  preflight) preflight ;;
  fresh) preflight; fresh_replay ;;
  upgrade) preflight; upgrade_replay ;;
  all) preflight; fresh_replay; upgrade_replay ;;
esac
