...
export const AGENT_SKILL_EVAL_FRAMEWORK_V2_EXACT_FILES = new Set([
...
export const agentSkillEvalFrameworkV2Commands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('syntax', 'node', ['--check', 'lib/agent-skill-evals/trainingos-agent-skill-evals-v2.mjs']),
  ...
  command('focused-python-contracts', 'python', [
    '-m', 'unittest', '-v',
    'tests.test_trainingos_agent_skill_eval_framework_v2',
  ], 'python'),
...
export function isAgentSkillEvalFrameworkV2Scope(files) {
  const names = [...files];
  return names.length === AGENT_SKILL_EVAL_FRAMEWORK_V2_EXACT_FILES.size
    && names.every((name) => AGENT_SKILL_EVAL_FRAMEWORK_V2_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: agentSkillEvalFrameworkV2Commands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunAgentSkillEvalFrameworkV2Profile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isAgentSkillEvalFrameworkV2Scope(files)) return null;
  ...
  try {
    for (const [index, item] of agentSkillEvalFrameworkV2Commands.entries()) {
  ...
  const stepCount = agentSkillEvalFrameworkV2Commands.length;
