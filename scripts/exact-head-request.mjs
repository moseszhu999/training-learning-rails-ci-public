...
async function main() {
  ...
  await appendFile(outputPath, [
    ...
    `expected_main_sha=${value.expectedMainSha}`,
    ...
    `expected_migration_count=${value.expectedMigrationCount}`,
    `run_fresh_replay=${value.runFreshReplay}`,
    `run_upgrade_replay=${value.runUpgradeReplay}`,
    `run_application_contracts=${value.runApplicationContracts}`,
    `run_typecheck=${value.runTypecheck}`,
    `run_production_build=${value.runProductionBuild}`,
    `run_critical_e2e=${value.runCriticalE2E}`,
