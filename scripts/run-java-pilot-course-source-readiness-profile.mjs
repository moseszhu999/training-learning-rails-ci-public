...
export const JAVA_PILOT_COURSE_SOURCE_READINESS_EXACT_FILES = new Set([
  'docs/architecture/trainingos-java-pilot-course-source-readiness-v1.md',
  'packages/training-industry-role-pack-core/reference-pilots/java-developer-course-source-inventory-v1.json',
  'packages/training-industry-role-pack-core/src/java-pilot-course-source-readiness.mjs',
  'packages/training-industry-role-pack-core/test/java-pilot-course-source-readiness.test.mjs',
  'tests/test_trainingos_java_pilot_course_source_readiness_v1.py',
]);
...
export const javaPilotCourseSourceReadinessCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('readiness-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/src/java-pilot-course-source-readiness.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/java-pilot-course-source-readiness.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_java_pilot_course_source_readiness_v1'], 'python'),
...
export function isJavaPilotCourseSourceReadinessScope(files) {
  const names = [...files];
  return names.length === JAVA_PILOT_COURSE_SOURCE_READINESS_EXACT_FILES.size
    && names.every((name) => JAVA_PILOT_COURSE_SOURCE_READINESS_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: javaPilotCourseSourceReadinessCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunJavaPilotCourseSourceReadinessProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isJavaPilotCourseSourceReadinessScope(files)) return null;
  ...
  try {
    for (const [index, item] of javaPilotCourseSourceReadinessCommands.entries()) {
  ...
  const stepCount = javaPilotCourseSourceReadinessCommands.length;
