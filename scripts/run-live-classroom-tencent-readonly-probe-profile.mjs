...
export const LIVE_CLASSROOM_TENCENT_READONLY_PROBE_EXACT_FILES = new Set([
  'docs/architecture/trainingos-live-classroom-tencent-readonly-probe-v1.md',
...
export const liveClassroomTencentReadonlyProbeCommands = Object.freeze([
...
export function isLiveClassroomTencentReadonlyProbeScope(files) {
  const names = [...files];
  return names.length === LIVE_CLASSROOM_TENCENT_READONLY_PROBE_EXACT_FILES.size
    && names.every((name) => LIVE_CLASSROOM_TENCENT_READONLY_PROBE_EXACT_FILES.has(name))
...
function failedResult(reason) {
  return {
    ...
    stepCount: liveClassroomTencentReadonlyProbeCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
    pythonTests: 0,
    selectedSuite: 'live-classroom-tencent-readonly-probe',
  };
}

export async function maybeRunLiveClassroomTencentReadonlyProbeProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isLiveClassroomTencentReadonlyProbeScope(files)) return null;
  ...
  try {
    for (const [index, item] of liveClassroomTencentReadonlyProbeCommands.entries()) {
  ...
  const ok = passedStepCount === liveClassroomTencentReadonlyProbeCommands.length
    && countsPassed
    && dryRunPassed;
