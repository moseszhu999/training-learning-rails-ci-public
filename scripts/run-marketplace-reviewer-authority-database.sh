...
on_error(){ echo "MARKETPLACE_REVIEWER_AUTHORITY_DB status=FAIL stage=$CURRENT_STAGE"; }
trap on_error ERR

required=(PRIVATE_REPO_PATH PRIVATE_EXACT_SHA EXPECTED_MIGRATION_COUNT RUNNER_TEMP)
for name in "${required[@]}"; do
  [[ -n "${!name:-}" ]] || { echo "MARKETPLACE_REVIEWER_AUTHORITY_DB status=FAIL stage=inputs"; exit 2; }
done

canonical_migration_count=365
base_migration_count=364
migration_name="20260804212000_trainingos_marketplace_reviewer_authority_owner_v1.sql"
...
[[ -f "$scope_file" ]] || { echo "MARKETPLACE_REVIEWER_AUTHORITY_DB status=FAIL stage=scope-contract"; exit 2; }
...
[[ "$(read_scope migration_start)" == "20260804212000" ]]
[[ "$(read_scope migration_end)" == "20260804212000" ]]

CURRENT_STAGE="supabase-wrapper"
bin_dir="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-bin"
mkdir -p "$bin_dir"
...
fresh="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-fresh"
upgrade="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-upgrade"
base_repo="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-base-repo"
...
cleanup(){
  ...
  rm -f "$RUNNER_TEMP"/trainingos-marketplace-reviewer-authority-*.env
}
trap cleanup EXIT

sealed(){
  local label="$1"; shift
  "$@" >"$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-${label}.log" 2>&1
}
...
sanitized_failure_marker(){
  ...
  line="$(grep -Eo 'trainingos_marketplace_reviewer_authority_owner_v1_e2e\.sql:[0-9]+' "$log_path" 2>/dev/null \
...
sanitized_start_failure_marker(){
  ...
  if grep -q "$migration_name" "$log_path" 2>/dev/null; then migration_scope="reviewer_authority"
  elif grep -Eq '20[0-9]{12}_[a-z0-9_]+\.sql' "$log_path" 2>/dev/null; then migration_scope="base_history"
  else migration_scope="none"; fi

  if grep -q 'FATAL:' "$log_path" 2>/dev/null; then category="fatal"
  elif grep -q 'ERROR:' "$log_path" 2>/dev/null; then category="error"
  elif grep -Eqi '(supabase|docker|container|psql):.*(error|failed)' "$log_path" 2>/dev/null; then category="client"
  else category="unknown"; fi
...
start_with_marker(){
  ...
  start_log="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-${label}.log"
  set +e
...
run_e2e(){
  ...
  status_file="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-${label}.env"
  ...
  e2e_log="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-${label}-sql-e2e.log"
  set +e
  psql "$db_url" -X -v ON_ERROR_STOP=1 -v VERBOSITY=verbose \
    -c 'begin;' \
    -f "$PRIVATE_REPO_PATH/tests/sql/trainingos_marketplace_reviewer_authority_owner_v1_e2e.sql" \
  ...
  grep -q 'TRAININGOS_MARKETPLACE_REVIEWER_AUTHORITY_OWNER_V1_E2E_PASS' "$e2e_log"

  CURRENT_STAGE="${label}-rollback-residue"
  residue_log="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-${label}-residue.log"
  psql "$db_url" -X -v ON_ERROR_STOP=1 -At <<'SQL' >"$residue_log" 2>&1
select 'fixtures=' || count(*)
from public.profiles
where employee_or_student_number in ('MKT-R-O-001', 'MKT-R-R-001', 'MKT-R-U-001');
select 'authority_events=' || count(*)
from public.trainingos_marketplace_reviewer_authority_events;
SQL
  grep -qx 'fixtures=0' "$residue_log"
  grep -qx 'authority_events=0' "$residue_log"

  CURRENT_STAGE="${label}-catalog"
  catalog_log="$RUNNER_TEMP/trainingos-marketplace-reviewer-authority-${label}-catalog.log"
  psql "$db_url" -X -v ON_ERROR_STOP=1 -v elevated_role="$elevated_role" -At <<'SQL' >"$catalog_log" 2>&1
select 'tables=' || count(*)
from pg_catalog.pg_class relation
join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
where namespace.nspname = 'public'
  and relation.relname = 'trainingos_marketplace_reviewer_authority_events'
...
where namespace.nspname = 'public'
  and proc.proname = 'get_my_trainingos_marketplace_reviewer_authority_v1'
  and proc.prosecdef
  and coalesce(array_to_string(proc.proconfig, ','), '') like '%search_path=%';

select 'private_guards=' || count(*)
from pg_catalog.pg_proc proc
join pg_catalog.pg_namespace namespace on namespace.oid = proc.pronamespace
where namespace.nspname = 'private'
  and proc.proname in (
    'grant_trainingos_marketplace_reviewer_authority_v1',
    'revoke_trainingos_marketplace_reviewer_authority_v1',
    'require_trainingos_marketplace_reviewer_authority_v1'
  )
  and proc.prosecdef
  and coalesce(array_to_string(proc.proconfig, ','), '') like '%search_path=%';
...
where table_schema = 'public'
  and table_name = 'trainingos_marketplace_reviewer_authority_events'
...
select 'network_operator_exec=' || count(*)
from pg_catalog.pg_proc proc
join pg_catalog.pg_namespace namespace on namespace.oid = proc.pronamespace
where namespace.nspname = 'private'
  and proc.proname in (
...
select 'forbidden_lifecycle_rpcs=' || count(*)
...
select 'forbidden_receipt_table=' || count(*)
from pg_catalog.pg_class relation
join pg_catalog.pg_namespace namespace on namespace.oid = relation.relnamespace
where namespace.nspname = 'public'
  and relation.relname = 'trainingos_marketplace_claim_review_receipts';
SQL
  grep -qx 'tables=1' "$catalog_log"
  grep -qx 'public_rpcs=1' "$catalog_log"
  grep -qx 'private_guards=3' "$catalog_log"
  ...
  grep -qx 'network_operator_exec=0' "$catalog_log"
  grep -qx 'forbidden_lifecycle_rpcs=0' "$catalog_log"
  grep -qx 'forbidden_receipt_table=0' "$catalog_log"
}
...
echo "MARKETPLACE_REVIEWER_AUTHORITY_DB status=PASS exact_head=$PRIVATE_EXACT_SHA canonical_migrations=$canonical_migration_count fresh_replay=PASS second_replay=PASS upgrade_replay=PASS sql_e2e=PASS rollback=PASS catalog=PASS zero_residue=PASS cleanup=PASS"
