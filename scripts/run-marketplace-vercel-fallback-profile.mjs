...
export const MARKETPLACE_VERCEL_FALLBACK_EXACT_FILES = new Set([
  'lib/trainingos-agent-gateway/vercel-route-dispatcher.mjs',
  'tests/trainingos-vercel-marketplace-fallback-v1.test.mjs',
  'vercel.json',
]);
...
export const marketplaceVercelFallbackCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('dispatcher-module-load', 'node', ['--input-type=module', '--eval', "await import('./lib/trainingos-agent-gateway/vercel-route-dispatcher.mjs')"]),
  command('focused-node-contracts', 'node', ['--test', 'tests/trainingos-vercel-marketplace-fallback-v1.test.mjs'], 'node'),
...
export function isMarketplaceVercelFallbackScope(files) {
  const names = [...files];
  return names.length === MARKETPLACE_VERCEL_FALLBACK_EXACT_FILES.size
    && names.every((name) => MARKETPLACE_VERCEL_FALLBACK_EXACT_FILES.has(name))
    && names.every((name) => !name.startsWith('supabase/migrations/'));
}

function failedContractResult() {
  return { ok: false, status: 'FAIL:fixed-input-contract', failedLabels: Object.freeze(['fixed-input-contract']), stepCount: marketplaceVercelFallbackCommands.length, passedStepCount: 0, nodeTests: 0, nodePassed: 0, nodeFailed: 0, pythonTests: 0, selectedSuite: 'marketplace-vercel-fallback' };
}

export async function maybeRunMarketplaceVercelFallbackProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isMarketplaceVercelFallbackScope(files)) return null;
  ...
  const failedLabels = [];
  try {
    for (const [index, item] of marketplaceVercelFallbackCommands.entries()) {
      const logPath = path.join(input.runnerTemp, `trainingos-marketplace-vercel-fallback-${index + 1}.log`);
  ...
  const stepCount = marketplaceVercelFallbackCommands.length;
  ...
  return { ok, status: ok ? 'PASS' : `FAIL:${failedLabels.length ? failedLabels.join(',') : 'count-contract'}`, failedLabels: Object.freeze([...failedLabels]), stepCount, passedStepCount, nodeTests, nodePassed, nodeFailed, pythonTests: 0, selectedSuite: 'marketplace-vercel-fallback' };
}
