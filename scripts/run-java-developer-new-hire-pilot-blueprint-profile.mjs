...
export const JAVA_DEVELOPER_NEW_HIRE_PILOT_EXACT_FILES = new Set([
  'docs/architecture/trainingos-java-developer-new-hire-pilot-blueprint-v1.md',
  'packages/training-industry-role-pack-core/reference-pilots/compile-java-developer-new-hire-pilot-v1.mjs',
  'packages/training-industry-role-pack-core/reference-pilots/java-developer-new-hire-pilot-v1.json',
  'packages/training-industry-role-pack-core/test/java-developer-new-hire-pilot.test.mjs',
  'tests/test_trainingos_java_developer_new_hire_pilot_blueprint_v1.py',
]);
...
export const javaDeveloperNewHirePilotCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('pilot-adapter-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/reference-pilots/compile-java-developer-new-hire-pilot-v1.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/java-developer-new-hire-pilot.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_java_developer_new_hire_pilot_blueprint_v1'], 'python'),
  command('pilot-compile-smoke', 'node', ['packages/training-industry-role-pack-core/reference-pilots/compile-java-developer-new-hire-pilot-v1.mjs']),
...
export function isJavaDeveloperNewHirePilotScope(files) {
  const names = [...files];
  return names.length === JAVA_DEVELOPER_NEW_HIRE_PILOT_EXACT_FILES.size
    && names.every((name) => JAVA_DEVELOPER_NEW_HIRE_PILOT_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: javaDeveloperNewHirePilotCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunJavaDeveloperNewHirePilotProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isJavaDeveloperNewHirePilotScope(files)) return null;
  ...
  try {
    for (const [index, item] of javaDeveloperNewHirePilotCommands.entries()) {
  ...
  const stepCount = javaDeveloperNewHirePilotCommands.length;
