...
export const ENTITLEMENT_BILLING_PROJECTION_EXACT_FILES = new Set([
  'docs/product/trainingos-entitlement-billing-projection-core-v1.md',
  'packages/trainingos-entitlement-billing-core/package.json',
  'packages/trainingos-entitlement-billing-core/src/index.d.ts',
  'packages/trainingos-entitlement-billing-core/src/index.mjs',
  'tests/trainingos-entitlement-billing-projection-core-v1.test.mjs',
]);
...
export const entitlementBillingProjectionCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('package-syntax', 'node', ['--check', 'packages/trainingos-entitlement-billing-core/src/index.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'tests/trainingos-entitlement-billing-projection-core-v1.test.mjs'], 'node'),
...
export function isEntitlementBillingProjectionScope(files) {
  const names = [...files];
  return names.length === ENTITLEMENT_BILLING_PROJECTION_EXACT_FILES.size
    && names.every((name) => ENTITLEMENT_BILLING_PROJECTION_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: entitlementBillingProjectionCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
    pythonTests: 0,
    selectedSuite: 'entitlement-billing-projection-core',
  };
}

export async function maybeRunEntitlementBillingProjectionProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isEntitlementBillingProjectionScope(files)) return null;
  ...
  try {
    for (const [index, item] of entitlementBillingProjectionCommands.entries()) {
  ...
  const countsPassed = nodeTests === EXPECTED_NODE_COUNT
    ...
    && nodeFailed === 0;
  const stepCount = entitlementBillingProjectionCommands.length;
