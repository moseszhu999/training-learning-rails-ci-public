...
export const INDUSTRY_ROLE_PACK_REGISTRY_EXACT_FILES = new Set([
  'docs/architecture/trainingos-industry-role-pack-registry-v1.md',
  'packages/training-industry-role-pack-core/reference-packs/reference-organization-role-profiles-v1.json',
  'packages/training-industry-role-pack-core/reference-packs/reference-role-packs-v1.json',
  'packages/training-industry-role-pack-core/src/registry.mjs',
  'packages/training-industry-role-pack-core/test/registry.test.mjs',
  'tests/test_trainingos_industry_role_pack_registry_v1.py',
]);
...
export const industryRolePackRegistryCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('registry-syntax', 'node', ['--check', 'packages/training-industry-role-pack-core/src/registry.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'packages/training-industry-role-pack-core/test/registry.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_industry_role_pack_registry_v1'], 'python'),
...
export function isIndustryRolePackRegistryScope(files) {
  const names = [...files];
  return names.length === INDUSTRY_ROLE_PACK_REGISTRY_EXACT_FILES.size
    && names.every((name) => INDUSTRY_ROLE_PACK_REGISTRY_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: industryRolePackRegistryCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunIndustryRolePackRegistryProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isIndustryRolePackRegistryScope(files)) return null;
  ...
  try {
    for (const [index, item] of industryRolePackRegistryCommands.entries()) {
  ...
  const stepCount = industryRolePackRegistryCommands.length;
