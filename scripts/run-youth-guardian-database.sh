...
[[ "$migration_start" == 20260730090000 && "$migration_end" == 20260730095959 ]]
...
expected_non_migrations="$(printf '%s\n' \
  ...
  tests/test_trainingos_youth_guardian_safety_v1_contract.py | sort)"
expected_migrations=(
...
runner_sql="$PRIVATE_REPO_PATH/tests/sql/trainingos_youth_guardian_safety_v1_e2e_runner.sql"

fresh_project="$RUNNER_TEMP/trainingos-youth-guardian-fresh"
...
grep -Fq '\ir trainingos_youth_guardian_safety_v1_e2e.sql' "$runner_sql"
