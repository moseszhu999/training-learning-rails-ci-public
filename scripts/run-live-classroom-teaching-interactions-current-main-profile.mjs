...
export const LIVE_CLASSROOM_TEACHING_INTERACTIONS_EXACT_FILES = new Set([
...
export const liveClassroomTeachingInteractionsCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_live_classroom_teaching_interactions_v1'], 'python'),
...
export function isLiveClassroomTeachingInteractionsScope(files) {
  const names = [...files];
  return names.length === LIVE_CLASSROOM_TEACHING_INTERACTIONS_EXACT_FILES.size
    && names.every((name) => LIVE_CLASSROOM_TEACHING_INTERACTIONS_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: liveClassroomTeachingInteractionsCommands.length, passedStepCount: 0,
    nodeTests: 0, nodePassed: 0, nodeFailed: 0, pythonTests: 0,
    selectedSuite: 'live-classroom-teaching-interactions-current-main',
  };
}

export async function maybeRunLiveClassroomTeachingInteractionsCurrentMainProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isLiveClassroomTeachingInteractionsScope(files)) return null;
  ...
  const failedLabels = [];
  try {
    for (const [index, item] of liveClassroomTeachingInteractionsCommands.entries()) {
  ...
  const stepCount = liveClassroomTeachingInteractionsCommands.length;
