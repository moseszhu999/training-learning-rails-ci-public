...
export const currentCapabilityCredentialReadCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('focused-node-contracts', 'node', ['--test', 'tests/training-current-capability-credential-read-v1.test.mjs'], 'node'),
...
export function isCurrentCapabilityCredentialReadScope(files) {
  const names = [...files];
  return names.length === CURRENT_CAPABILITY_CREDENTIAL_READ_EXACT_FILES.size
    && names.every((name) => CURRENT_CAPABILITY_CREDENTIAL_READ_EXACT_FILES.has(name));
}
...
function failedContractResult() {
  return {
    ...
    stepCount: currentCapabilityCredentialReadCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunCurrentCapabilityCredentialReadProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isCurrentCapabilityCredentialReadScope(files)) return null;
  ...
  try {
    for (const [index, item] of currentCapabilityCredentialReadCommands.entries()) {
      const logPath = path.join(input.runnerTemp, `trainingos-current-capability-credential-read-${index + 1}.log`);
  ...
  const stepCount = currentCapabilityCredentialReadCommands.length;
