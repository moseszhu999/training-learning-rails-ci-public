...
export const MARKETPLACE_SUPPLIER_CONTACT_RESPONSE_UI_EXACT_FILES = new Set([
...
export const marketplaceSupplierContactResponseUiCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('supplier-response-runtime-syntax', 'node', [
    '--check',
    'apps/training-marketplace-web/src/supplier-contact-response-runtime.mjs',
  ]),
  ...
  command('node-supplier-response-ui', 'node', [
    '--test',
    'apps/training-marketplace-web/test/supplier-contact-response-runtime.test.mjs',
  ], 'node'),
  command('python-static', 'python', [
    ...
    'unittest',
    '-v',
    'tests.test_trainingos_marketplace_supplier_contact_response_ui_v1',
  ], 'python'),
...
export function isMarketplaceSupplierContactResponseUiScope(files) {
  const names = [...files];
  return names.length === MARKETPLACE_SUPPLIER_CONTACT_RESPONSE_UI_EXACT_FILES.size
    && names.every((name) => MARKETPLACE_SUPPLIER_CONTACT_RESPONSE_UI_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: marketplaceSupplierContactResponseUiCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunMarketplaceSupplierContactResponseUiProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isMarketplaceSupplierContactResponseUiScope(files)) return null;
  ...
  try {
    for (const [index, item] of marketplaceSupplierContactResponseUiCommands.entries()) {
  ...
  const ok = passedStepCount === marketplaceSupplierContactResponseUiCommands.length && countsPassed;
