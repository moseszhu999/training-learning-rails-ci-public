...
export const CAPABILITY_LEARNING_PROFILE_CORE_EXACT_FILES = new Set([
  'docs/architecture/trainingos-capability-learning-profile-v1.md',
  'packages/training-capability-learning-profile-core/package.json',
  'packages/training-capability-learning-profile-core/src/index.d.ts',
  'packages/training-capability-learning-profile-core/src/index.mjs',
  'tests/training-capability-learning-profile-v1.test.mjs',
]);
...
export const capabilityLearningProfileCoreCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('contract-syntax', 'node', ['--check', 'packages/training-capability-learning-profile-core/src/index.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'tests/training-capability-learning-profile-v1.test.mjs'], 'node'),
...
export function isCapabilityLearningProfileCoreScope(files) {
  const names = [...files];
  return names.length === CAPABILITY_LEARNING_PROFILE_CORE_EXACT_FILES.size
    && names.every((name) => CAPABILITY_LEARNING_PROFILE_CORE_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: capabilityLearningProfileCoreCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunCapabilityLearningProfileCoreProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isCapabilityLearningProfileCoreScope(files)) return null;
  ...
  try {
    for (const [index, item] of capabilityLearningProfileCoreCommands.entries()) {
  ...
  const countsPassed = nodeTests === EXPECTED_NODE_COUNT
    ...
    && nodeFailed === 0;
  const stepCount = capabilityLearningProfileCoreCommands.length;
