...
export const MARKETPLACE_PARTICIPATION_CLIENT_EXACT_FILES = new Set([
...
export const marketplaceParticipationClientCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('node-adapter', 'node', ['--test', 'packages/training-marketplace-participation-client/test/participation-client.test.mjs'], 'node'),
  command('python-static', 'python', ['tests/test_trainingos_marketplace_participation_client_adapter_v1.py'], 'python'),
  command('package-syntax', 'node', ['--check', 'packages/training-marketplace-participation-client/src/index.mjs']),
  command('declaration-typecheck', 'npx', ['tsc', '--strict', '--noEmit', '--skipLibCheck', '--target', 'ES2022', '--module', 'ESNext', '--moduleResolution', 'Bundler', '--lib', 'ES2022,DOM', 'packages/training-marketplace-participation-client/src/index.d.ts']),
...
export function isMarketplaceParticipationClientScope(files) {
  const names = [...files];
  return names.length === MARKETPLACE_PARTICIPATION_CLIENT_EXACT_FILES.size
    && names.every((name) => MARKETPLACE_PARTICIPATION_CLIENT_EXACT_FILES.has(name))
...
export async function maybeRunMarketplaceParticipationClientProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isMarketplaceParticipationClientScope(files)) return null;
  if (!fixedInputContract(input, scope)) {
    await rm(path.join(input.runnerTemp, 'trainingos-scope-contract.env'), { force: true });
    return { ok: false, status: 'FAIL:fixed-input-contract', failedLabels: Object.freeze(['fixed-input-contract']), stepCount: marketplaceParticipationClientCommands.length, passedStepCount: 0, nodeTests: 0, nodePassed: 0, nodeFailed: 0, pythonTests: 0, selectedSuite: 'marketplace-participation-client' };
  }
  ...
  const failedLabels = [];
  try {
    for (const [index, item] of marketplaceParticipationClientCommands.entries()) {
  ...
  const ok = passedStepCount === marketplaceParticipationClientCommands.length && countsPassed;
  return { ok, status: ok ? 'PASS' : `FAIL:${failedLabels.length ? failedLabels.join(',') : 'count-contract'}`, failedLabels: Object.freeze([...failedLabels]), stepCount: marketplaceParticipationClientCommands.length, passedStepCount, nodeTests, nodePassed, nodeFailed, pythonTests, selectedSuite: 'marketplace-participation-client' };
}
