...
export const profileCommands = Object.freeze({
  ...
  'challenge-runtime': [
    ...
    command('production-build', 'npm', ['run', 'build']),
    command('database-preflight', 'bash', [path.join(publicRoot, 'scripts/run-challenge-runtime-database.sh'), 'preflight']),
    command('database-fresh', 'bash', [path.join(publicRoot, 'scripts/run-challenge-runtime-database.sh'), 'fresh']),
    command('database-upgrade', 'bash', [path.join(publicRoot, 'scripts/run-challenge-runtime-database.sh'), 'upgrade']),
  ],
  'challenge-web': [
    command('install', 'npm', ['ci']),
