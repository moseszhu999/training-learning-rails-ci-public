...
export const MARKETPLACE_AUTHENTICATED_CONTACT_UI_EXACT_FILES = new Set([
...
export const marketplaceAuthenticatedContactUiCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('contact-runtime-syntax', 'node', [
    '--check',
    'apps/training-marketplace-web/src/authenticated-contact-runtime.mjs',
  ]),
  ...
  command('python-static', 'python', [
    ...
    'unittest',
    '-v',
    'tests.test_trainingos_marketplace_authenticated_contact_ui_v1',
  ], 'python'),
...
export function isMarketplaceAuthenticatedContactUiScope(files) {
  const names = [...files];
  return names.length === MARKETPLACE_AUTHENTICATED_CONTACT_UI_EXACT_FILES.size
    && names.every((name) => MARKETPLACE_AUTHENTICATED_CONTACT_UI_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: marketplaceAuthenticatedContactUiCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunMarketplaceAuthenticatedContactUiProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isMarketplaceAuthenticatedContactUiScope(files)) return null;
  ...
  try {
    for (const [index, item] of marketplaceAuthenticatedContactUiCommands.entries()) {
  ...
  const ok = passedStepCount === marketplaceAuthenticatedContactUiCommands.length && countsPassed;
