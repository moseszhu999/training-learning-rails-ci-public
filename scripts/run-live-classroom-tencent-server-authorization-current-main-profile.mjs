...
export const LIVE_CLASSROOM_F2_CURRENT_MAIN_EXACT_FILES = new Set([
...
export const liveClassroomF2CurrentMainCommands = Object.freeze([
  ...
  command('focused-node-contracts', 'node', ['--test', 'prototypes/trainingos-agent-mvp-v1/test/tencent-live-classroom-server-authorization.test.mjs'], 'node'),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_live_classroom_tencent_server_authorization_v1'], 'python'),
...
export function isLiveClassroomF2CurrentMainScope(files) {
  const names = [...files];
  return names.length === LIVE_CLASSROOM_F2_CURRENT_MAIN_EXACT_FILES.size
    && names.every((name) => LIVE_CLASSROOM_F2_CURRENT_MAIN_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: liveClassroomF2CurrentMainCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunLiveClassroomF2CurrentMainProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isLiveClassroomF2CurrentMainScope(files)) return null;
  ...
  try {
    for (const [index, item] of liveClassroomF2CurrentMainCommands.entries()) {
  ...
  const stepCount = liveClassroomF2CurrentMainCommands.length;
