...
export const GROUP_WORK_ENTRY_ADAPTER_EXACT_FILES = new Set([
  'docs/architecture/trainingos-group-work-entry-adapter-v1.md',
  'packages/training-group-work-entry-adapter/package.json',
  'packages/training-group-work-entry-adapter/src/index.d.ts',
...
export const groupWorkEntryAdapterCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('contract-syntax', 'node', ['--check', 'packages/training-group-work-entry-adapter/src/index.mjs']),
  command('focused-node-contracts', 'node', ['--test', 'tests/training-group-work-entry-adapter-v1.test.mjs'], 'node'),
...
export function isGroupWorkEntryAdapterScope(files) {
  const names = [...files];
  return names.length === GROUP_WORK_ENTRY_ADAPTER_EXACT_FILES.size
    && names.every((name) => GROUP_WORK_ENTRY_ADAPTER_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: groupWorkEntryAdapterCommands.length,
    passedStepCount: 0,
    nodeTests: 0,
    nodePassed: 0,
    nodeFailed: 0,
...
export async function maybeRunGroupWorkEntryAdapterProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isGroupWorkEntryAdapterScope(files)) return null;
  ...
  try {
    for (const [index, item] of groupWorkEntryAdapterCommands.entries()) {
      const logPath = path.join(input.runnerTemp, `trainingos-group-work-entry-adapter-${index + 1}.log`);
  ...
  const stepCount = groupWorkEntryAdapterCommands.length;
