...
export const LIVE_CLASSROOM_POSTCLASS_EVIDENCE_EXACT_FILES = new Set([
...
export const liveClassroomPostclassEvidenceCommands = Object.freeze([
  command('install', 'npm', ['ci']),
  command('focused-python-contracts', 'python', ['-m', 'unittest', '-v', 'tests.test_trainingos_live_classroom_postclass_evidence_v1'], 'python'),
...
export function isLiveClassroomPostclassEvidenceScope(files) {
  const names = [...files];
  return names.length === LIVE_CLASSROOM_POSTCLASS_EVIDENCE_EXACT_FILES.size
    && names.every((name) => LIVE_CLASSROOM_POSTCLASS_EVIDENCE_EXACT_FILES.has(name))
...
function failedContractResult() {
  return {
    ...
    stepCount: liveClassroomPostclassEvidenceCommands.length, passedStepCount: 0,
    nodeTests: 0, nodePassed: 0, nodeFailed: 0, pythonTests: 0,
    selectedSuite: 'live-classroom-postclass-evidence-current-main',
  };
}

export async function maybeRunLiveClassroomPostclassEvidenceCurrentMainProfile(input) {
  if (input.profile !== 'generic-owned') return null;
  const { files, scope } = await exactChangedFiles(input);
  if (!isLiveClassroomPostclassEvidenceScope(files)) return null;
  ...
  const failedLabels = [];
  try {
    for (const [index, item] of liveClassroomPostclassEvidenceCommands.entries()) {
  ...
  const stepCount = liveClassroomPostclassEvidenceCommands.length;
