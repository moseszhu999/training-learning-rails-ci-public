...
PRIVATE_SHA="ce66d6d388b6f6353e3b7967f392dec58e31a551"
...
run_application() {
  ...
  node --test \
    ...
    > "$node_log" 2>&1
  python - "$node_log" <<'PY' > "$SEALED_DIR/pr339-node-summary.txt"
import pathlib, re, sys
text = pathlib.Path(sys.argv[1]).read_text(encoding='utf-8', errors='replace')
def n(pattern, default):
    match = re.search(pattern, text)
    return int(match.group(1)) if match else default
tests = n(r'# tests\s+(\d+)', 0)
passed = n(r'# pass\s+(\d+)', 0)
failed = n(r'# fail\s+(\d+)', -1)
...
root = Path('.')
transition = (root / 'supabase/migrations/20260728100250_trainingos_student_exercise_rpc_transition_bridge_v1.sql').read_text()
...
gates = {
    'no_second_fact_table': 'create table' not in (transition + reconciliation + privacy + options).lower(),
    'record_assignment_fixed': 'select p, d into v_publication, v_definition' not in reconciliation.lower(),
    'transition_is_purpose_bound': all(token in transition for token in (
        'trainingos.student_exercise_rpc',
        'validate_exercise_submission_rpc_scope',
        'validate_exercise_submission_scope',
        'TRAININGOS_STUDENT_EXERCISE_RPC_TEACHER_FIELDS_IMMUTABLE',
    )),
    'direct_table_privilege_not_restored': 'grant insert' not in transition.lower() and 'grant update' not in transition.lower(),
    ...
    'privacy_strips_student_projection': all(token in privacy for token in (
    ...
    'option_compat_reuses_answer_trigger': 'create or replace function trainingos_assessment_private.validate_answer()' in options,
    'option_compat_accepts_bounded_identifiers': all(token in options for token in (
    ...
    'grading_bridge_transactional': grade_bridge.lstrip().startswith('-- TrainingOS W3A grading/result regression') and '\\ir trainingos_wave3_assessment_grading_result_e2e.sql' in grade_bridge and 'commit;' not in grade_bridge.lower(),
    'grading_bridge_uses_current_contract': all(token in grade_bridge for token in (
        ...
        'p_expected_answer_versions jsonb',
    )),
...
PY
  ...
  {
    echo 'exact_private_sha=ce66d6d388b6f6353e3b7967f392dec58e31a551'
...
run_sql_e2es() {
  local pass="$1"
  psql "$DB_URL" -X -qAt -v ON_ERROR_STOP=1 -f tests/sql/trainingos_student_exercise_canonical_reconciliation_e2e.sql > "$SEALED_DIR/pr339-exercise-${pass}.log" 2>&1
  psql "$DB_URL" -X -qAt -v ON_ERROR_STOP=1 -f tests/sql/trainingos_student_learning_canonical_reconciliation_e2e.sql > "$SEALED_DIR/pr339-assessment-${pass}.log" 2>&1
  psql "$DB_URL" -X -qAt -v ON_ERROR_STOP=1 -f tests/sql/trainingos_wave3_assessment_grading_result_canonical_bridge_e2e.sql > "$SEALED_DIR/pr339-grading-${pass}.log" 2>&1
}
...
run_database() {
  ...
  python scripts/build-trainingos-fresh-bootstrap.py --repo-root "$PRIVATE_REPO" --output-dir "$workdir/supabase/migrations" --commit-sha "$PRIVATE_SHA" > "$SEALED_DIR/pr339-db-bootstrap.log" 2>&1
