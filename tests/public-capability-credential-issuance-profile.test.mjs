...
test('profile accepts exactly the seven bounded private files', () => {
  assert.equal(CAPABILITY_CREDENTIAL_ISSUANCE_EXACT_FILES.size, 7);
  ...
  assert.equal([...CAPABILITY_CREDENTIAL_ISSUANCE_EXACT_FILES].filter((name) => name.startsWith('supabase/migrations/')).length, 2);
});
...
test('profile source fixes expected tests and exact two-migration range without runtime DB commands', () => {
  const source = readFileSync(new URL('../scripts/run-capability-credential-issuance-profile.mjs', import.meta.url), 'utf8');
  assert.match(source, /EXPECTED_CHANGED_FILE_COUNT = 7/);
  assert.match(source, /EXPECTED_NODE_COUNT = 10/);
  assert.match(source, /EXPECTED_MIGRATION_COUNT = 376/);
  assert.match(source, /MIGRATION_START = '20260810093000'/);
  assert.match(source, /MIGRATION_END = '20260810094500'/);
