...
test('publishes exactly three fixed competitor experiment profiles', () => {
  ...
  assert.deepEqual(COMPETITOR_EXPERIMENT_PROFILES.map((profile) => profile.expectedNodeCount), [9, 9, 8]);
  assert.ok(COMPETITOR_EXPERIMENT_PROFILES.every((profile) => profile.exactFiles.size === 4));
});
...
test('each fixed profile runs only sealed install/syntax/focused-test/typecheck/build checks', () => {
  for (const profile of COMPETITOR_EXPERIMENT_PROFILES) {
    assert.deepEqual(profile.commands.map((item) => item.label), [
      'install',
      profile.id === 'assistance-aware-roleplay' ? 'roleplay-syntax'
        : profile.id === 'verified-skill-evidence' ? 'verified-skill-syntax'
          : 'expert-skill-syntax',
