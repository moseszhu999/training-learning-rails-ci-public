...
test('database controller supports no-migration post-merge replay safely', async () => {
  ...
  for (const marker of [
    ...
    "manifest.get('sourceCommitSha') == source_sha",
    "manifest.get('compatibilityMigrationCount', -1)",
    'migration_count == staged_count',
    'migration_count == source_count + compatibility_count',
    'second_pass=PASS',
