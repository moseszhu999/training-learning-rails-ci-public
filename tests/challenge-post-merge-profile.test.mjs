...
import { isChallengePostMergeV2Files } from '../scripts/run-private-profile-stage3.mjs';
...
test('post-merge v2 suite selection is exact and fails closed', () => {
  assert.equal(isChallengePostMergeV2Files(expectedFiles), true);
  assert.equal(isChallengePostMergeV2Files([...expectedFiles].reverse()), true);
  assert.equal(isChallengePostMergeV2Files(expectedFiles.slice(0, 2)), false);
  assert.equal(isChallengePostMergeV2Files([...expectedFiles, 'README.md']), false);
});
...
test('post-merge profile exposes only fixed Learning Workspace and database stages', () => {
  ...
  assert.match(serialized, /test_trainingos_student_exercise_execution_contract\.py/);
...
test('database controller validates generated manifest count and no-migration replay safely', async () => {
